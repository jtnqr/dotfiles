...
if ($env:SSH_CONNECTION) {
    $EZA_BIN    = "$env:USERPROFILE\scoop\apps\eza\current\eza.exe"
    $FNM_BIN    = "$env:USERPROFILE\scoop\apps\fnm\current\fnm.exe"
    $ZOXIDE_BIN = $null
    $NPP_BIN    = $null
    $SUBL_BIN   = $null
} else {
    ...
    $ZOXIDE_BIN = "zoxide"
    $FNM_BIN    = "fnm"
    $NPP_BIN    = "$env:USERPROFILE\scoop\apps\notepadplusplus\current\notepad++.exe"
    $SUBL_BIN   = "$env:USERPROFILE\scoop\apps\sublime-text\current\subl.exe"
}
...
# Local/GUI-only configuration
if (-not $env:SSH_CONNECTION) {
    $env:EDITOR = $NPP_BIN
    Set-Alias -Name sudo -Value gsudo
    function choco { gsudo choco $args }
    function npp   { & $NPP_BIN $args }
    function subl  { & $SUBL_BIN $args }
}
